# Source: https://github.com/vimalloc/flask-jwt-extended/blob/master/examples/database_blacklist/blacklist_helpers.py
...
from app.lib.exceptions import TokenNotFound
from app import db
...
def revoke_token(token_jti, user):
    ...
    """
    try:
        token = TokenBlacklist.query.filter_by(jti=token_jti, user_identity=user).one()
        token.revoked = True
        db.session.commit()
        print ("Revoked Token")
    except NoResultFound:
